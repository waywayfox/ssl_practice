...
int main(int argc, char *argv[])
{
  ...
  {
    perror("Unabled to create listening socket.");
    exit(0);
  }

  if (setsockopt(listen_sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) == -1)
  {
    perror("Unabled to set socket option.");
    exit(0);
  }
  ...
  {
    perror("Unabled to bind to tlocal address.");
    exit(0);
  }

  if (listen(listen_sock, 5) == -1)
  {
    perror("Unabled to set socket backlog.");
    exit(0);
  }
  ...
  if (connect_sock == -1)
  {
    perror("Unabled to accept socket.");
  }
  return 0;
}
